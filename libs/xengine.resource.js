	...
	xengine.fn.extend(xengine, {
		...
		ResManager:{
			...
			getAnimationsByName:function(fResName,fName){
				var obj = this.res[FrameRes.ClassName][fResName];
			...
			parseRes:function(res,loadedFN,perLoadedFN){
				...
				var loadObj = null;
				var self = this;
				var loadHand = root.setInterval(function(){
					if(loadObj == null){
		...
		FrameRes:{
			load:function(name,url,loadedFN){
				//解析frameJSON格式配置文件
				var parse = function(animations,data){
					switch(data.type){
						case 0:
							var res = ResManager.getResByName(ImageRes.ClassName,data.img);
							var fs = new xengine.Frames("def",res.hEle);
							...
							if(fs == null){
								...
								for(var fname in fs){
									if(fs.hasOwnProperty(fname)){
										var fss = fs[fname];
										var fm = new xengine.Frames(fname,res.hEle);
				...
				ResUtil.loadFile(url,null,function(data){
					...
					for(var i in data){
						if(data.hasOwnProperty(i)){
							var f = data[i];
							obj.frames[i] = new xengine.Animations();
							//解析帧动画资源
